...
- 回答欄ごとに「1行・改行なし」または「複数行・改行あり」を選択
- 編集した `script.json` をPDF・音声と一緒に新しいZIPとして保存
- 配置モードを終了すると、作った回答欄へ直接入力
- 指定したページで、タイトルと長文本文を書ける作文エディターを表示
- 作文は文字数を確認しながら自動保存し、TXTとしても書き出し
...
作文させたいページへ `composition` を追加します。設定したページでは、上部に **「作文を書く」** ボタンが表示されます。タイトルと本文は入力するたびブラウザへ自動保存され、**「TXTを保存」** で作文だけをテキストファイルにできます。既存の **「回答を保存」** で書き出すJSONにも作文が含まれます。

```json
{
  "page": 25,
  "cues": [
    {"type": "speak", "text": "最後に、物語の本文を書きましょう。"},
    {"type": "wait", "label": "書き終わったら続けてください。"}
  ],
  "composition": {
    "prompt": "これまでの設計を見ながら、怖い話の本文を書きましょう。",
    "titlePlaceholder": "作品タイトル",
    "bodyPlaceholder": "ここに本文を書きます。段落ごとに改行できます。",
    "filename": "怖い話の作文",
    "maxLength": 50000
  }
}
...
| 項目 | 内容 |
|---|---|
| `prompt` | 作文画面の上部に表示する案内 |
| `titlePlaceholder` | タイトル欄が空のときの案内 |
| `bodyPlaceholder` | 本文欄が空のときの案内 |
| `filename` | TXT保存時のファイル名（拡張子は自動） |
| `maxLength` | 本文の最大文字数 |

`examples/horror-workbook-script.json` では25ページ目だけに作文画面を設定しています。
...
| 「作文を書く」 | 設定されたページで長文作文画面を開く |
| 「TXTを保存」 | 現在の作文をテキストファイルで保存 |
